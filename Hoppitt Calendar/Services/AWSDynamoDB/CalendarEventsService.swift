    ...
    func sendEventNotification(event: CalendarEvent, eventAction: String) {
        for (SNSEndpointARN, SNSEndpointToken) in SNS_DICT {
            ...
            if (!deviceArn.isEmpty && !deviceTokenSNS.isEmpty) {
                ...
                let dict = [
                    "APNS": "{\"aps\":{\"alert\": {\"title\":\"Event for \(event.who) on \(eventDay) \(eventMonth) \(eventAction)\",\"body\":\"\(event.title) starting at \(event.startDate.formatted(date: .omitted, time: .shortened))\"}} }"
                ]
